...
import mines.sim.SharedTimeEstimator;
import lpsolve.*;
import java.util.*;
...
public class LPFlowConstructor {
	...
	private static final double LARGECONSTANT = 1000;
	...
	private double[] emptyTimesMean;
	//average emptying times at each crusher location divided by the number of crushers at the location.
	private double[] fillTimesMean;			//average filling times for each shovel.
	private double[][] roadTravelTimesMean;	//average travelling times on each road in both direction.
	 ...
	 * @param numCrushers			the number of crusher locations.
	 * @param numShovels			the number of shovels.
	 * @param numRoads				the number of roads.
	 * @param emptyTimesMean		an array of average emptying times at each crusher location,
	 *								divided by the number of crushers at the location.
	 ...
	 * @param roadTravelTimesSD		a 2D array of travel times standard deviations on each road in both directions.
	...
	public LPFlowConstructor(int numTrucks, int numCrushers, int numShovels, int numRoads, double[] emptyTimesMean, double[] fillTimesMean, 
		...
		int[][] routeRoads, int[][] routeDirections, int[] routeLengths, int[] routeCrushers, int[] routeShovels) {
	...
	private LpSolve getLPModelScale(int[][] routeVariables) throws LpSolveException {
		...
		for (int i=0; i<numRoutes; i++) {
			double routeTime = 0;
			double reverseRouteTime = 0;
			for (int j=0; j<routeLengths[i]; j++) {
				int road = routeRoads[i][j];
				int dir = routeDirections[i][j];
				if (isOneWay[road]) {
					double[] expTimes = SharedTimeEstimator.estimateTravelTimes(new double[]{roadTravelTimesMean[road][dir],
						roadTravelTimesMean[road][1 - dir] * fullSlowdown},new double[]{maxExpRoadFlow[road],maxExpRoadFlow[road]},
						new double[]{roadTravelTimesSD[road][dir],roadTravelTimesSD[road][1 - dir] * fullSlowdown});
					routeTime += expTimes[0];
					reverseRouteTime += expTimes[1];
				}
				else {
					routeTime += roadTravelTimesMean[road][dir];
					reverseRouteTime += roadTravelTimesMean[road][1 - dir] * fullSlowdown;
				}
			}
			mult[numCrushers + numShovels + 2 * i] = routeTime;
			mult[numCrushers + numShovels + 2 * i + 1] = reverseRouteTime;
			for (int j=0; j<2; j++) {
